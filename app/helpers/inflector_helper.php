 ...
 * Capitular
 *
 * Transforma string separada por espaço em palavras com a primeira letra
 * em caixa-alta. Pode limitar o tamanho das strings transformadas e adicionar
 * dicionário de exceção.
 *
 * @access    public
 * @param    string
 * @return    str
...
if (!function_exists('capitular')) {
    ...
    function capitular($str, $maiorque = 2, $excecao = array()) {
        ...
        foreach ($lista as $w) {
            ...
            // verifica se a palavra está no linite indicado
            if (strlen($w) > 0) {

                if (strlen($w) > $maiorque) {

                    // verifica se existe dicionário de exceções
                    if (count($excecao) > 0) {

                        $busca = array_search($w, $excecao);
                    }

                    // se NÃO existe resulado positivo capitaliza
                    if (!isset($busca) || $busca === FALSE) {

                        $saida[] = ucfirst($w);
                    }
                    // senão não capitaliza
                    else {
