...
//        log_message('debug', 'notificationPost() do PagSeguro.');
        $postdata = 'Comando=validar&Token=E7F9554D3AC74EA49D9928641937F080';
...
//        curl_setopt($curl, CURLOPT_HTTPHEADER, Array("Content-Type: application/xml; charset=ISO-8859-1"))
    ...
    // -------------------------------------------------------------------------
    /** * OPCIONAL **
     * Faz consulta para receber o status da transação. Fonte: internet.
     * @param type $transacao
     * @return type
     */
    public function notification_status($transacao = NULL){

        if($transacao === NULL){
            $transacao = $_POST['notificationCode'];
        }
        
        $url = "https://ws.pagseguro.uol.com.br/v2/transactions/notifications/";
        $url .= $transacao . "?email=" . $this->ps_email . "&token=" . $this->token;
...
//    }
        ...
        // Estes status estão replicados em config/loja.php
        $status = $this->ci->config->item('status_transacao');

        return array(
            'code' => (int)$xml->status,
            'status' => $status[(int)$xml->status],
...
//                throw new Exception("CURL can't connect: $errorMessage");
    ...
    public function get_button($config = NULL){ 
        ...
        // primeira coisa, parsear as confgurações
     ...
     */
    public function set_user($user_id_array){
        
        $this->ci = &get_instance();
        
        // passou o ID do usuário
        if(is_numeric($user_id_array)){
            
            $this->ci->load->library('cms_usuario');            
            $user_array = $this->user_parser($this->ci->cms_usuario->get($user_id_array));
            
        } else {
            $user_array = $this->user_parser($user_id_array);
        }
    ...
    // -------------------------------------------------------------------------
    /**
     * O campo extra é usado para adicionar ou reduzir o valor do carrinho
     * no PagSeguro.
     * @param type $valor
     */
    public function set_desconto($valor = false){
        
        if($valor){
            $this->extras = '-'.(string)$valor;            
        }
...
//            $id = count($this->this_cart) + 1;
//            
//            $f[] = '<input type="hidden" name="itemId'.$id.'" value="'.$this->extras['id'].'">';
//            $f[] = '<input type="hidden" name="itemDescription'.$id.'" value="'.$this->extras['descricao'].'">';  
//            $f[] = '<input type="hidden" name="itemAmount'.$id.'" value="'.$this->extras['valor'].'">';  
//            $f[] = '<input type="hidden" name="itemQuantity'.$id.'" value="'.$this->extras['quantidade'].'">';  
//            $f[] = '<input type="hidden" name="itemWeight'.$id.'" value="'.$this->extras['peso'].'">';
//            
//            return implode("\n", $f);
            
            return '<input type="hidden" name="extraAmount" value="'.$this->extras.'">';
        }
    ...
    private function user_parser($user_array){
        ...
        foreach($user_array as $c => $v){
            ...
            // nome
//            if($c == 'nome' || $c == 'logradouro' || $c == 'cidade'){
//                $v = utf8_decode($v);
//            }
