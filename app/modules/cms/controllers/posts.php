...
class Posts extends Cms_Controller {
    ...
    function __construct()
    {
        ...
        $this->load->model(array('cms/posts_model', 'cms/admin_model', 'cms/paginas_model', 'cms/calendario_model'));
    ...
    function index($_var = '')
    {
         ...
         */
        $saida = $this->posts_model->lista_conteudos($this->var, 'conteudo', $modulo); // lista do conteúdo
//        mybug($saida['rows'][0]['grupoParents']);
        $dados['rows'] = $saida['rows']; // dados de conteúdo
        $dados['linkEditar'] = 'cms/posts/edita/co:' . $this->var['co'];
        $dados['linkFilter'] = 'cms/posts/index/co:' . $this->var['co'];
        /*
         * TEMPLATE
         */
...
//     $this->posts_model->setMetas(array('meta_key' => 'graph', 'meta_type'=> '', 'meta_value' => ''));
//        dd($this->posts_model->getMetas());
        /*
         * VARIÁVEIS
         */
        ...
        $this->tit_css = 'novo';
        $modulo = $this->modulo; // infos do módulo
        $conteudo = $this->posts_model->conteudo_dados($this->var); // dados deste conteúdo reprocessados
        ...
        /*
         * Seta opções para imagem opcional
         */
//        $this->load->library('widgets/cms_img_choose');
//        $this->cms_img_choose->registerWidget($this, array(
//            'model' => $this->posts_model,
//            'meta_key' => 'my-test-graph',
//            'img_folder' => 'assets/img/charts',
...
//        ));
        ...
        $this->options_menu->addDuplicate($this->var['co'], $this->var['id']);
...
//        mybug($this->dados['rel']);
    ...
    function salva()
    {
        ...
        {
            $ret = $this->posts_model->conteudo_salva($var);
            ...
            if ($ret)
            {
                redirect('cms/posts/edita/id:' . $ret . '/co:' . $co . '/tip:ok/tab:' . $tab);
            }
            else
            {
                redirect('cms/posts/edita/id:' . $ret . '/co:' . $co . '/tip:erro/tab:' . $tab);
            }
        }
    }
    ...
    function salva_copia()
    {
        ...
        if ($this->form_validation->run() == false)
        {
            ...
            redirect('cms/posts/duplicar/co:' . $co . '/id:' . $id . '/tip:erro');
        }
        // OK VÁLIDO
        else
        {

            $ret = $this->posts_model->conteudo_salva_copia($var);

            if ($ret)
            {
                redirect('cms/posts/edita/id:' . $ret . '/co:' . $co . '/tip:ok');
            }
            else
            {
                redirect('cms/posts/duplicar/tip:erro/co:' . $co . '/id:' . $id);
            }
        }
    }
...
//        $dados['rel'] = $this->cms_libs->combo_relacionados($this->modulo);
    ...
    function grupos($_var = '')
    {
        ...
        $conteudo = $this->posts_model->lista_grupos($this->var); // dados deste conteúdo reprocessados
        $conteudo = $this->posts_model->flatMultidimensionalArray($conteudo, 'sub'); // flat hierarchy
//        mybug($conteudo, true);
        ...
        $dados['linkEditar'] = 'cms/posts/grupoEdita/co:' . $this->var['co'];
...
//        $this->corpo = $this->layout_cms->barra_navegacao($this->var, $conteudo['ttl_rows'], $navegaOff);
    ...
    function grupoEdita($_var = '')
    {
        ...
        $this->cmsJS = array('tabs-forms', 'conteudo');
        $this->css = array();
        $this->setNewPlugin(array('colorpicker', 'nyromodal'));
        ...
        $this->dados['tipoGrupo'] = 'conteudo';
        $this->dados['rel'] = $this->posts_model->getGrupoComboHierarchy($this->var);
        $this->dados['linkReload'] = 'cms/posts/grupoEdita/co:' . $this->var['co'] . '/id:' . $this->var['id'];

//        mybug($this->dados['row'], true);

        /*
         * TABS
         */
        $this->tabs['tab_title'][] = $this->conteudo['titulo'];
        $this->tabs['tab_contt'][] = $this->load->view('cms/grupo_edita', $this->dados, true);
...
//        $this->corpo = $this->load->view('cms/template_tabs', $this->dados, true);
    ...
    function grupoSalva()
    {
        ...
        {
            $ret = $this->posts_model->grupo_salva($var);
            if ($id == '')
            {
                if ($ret)
                    redirect('cms/posts/grupos/co:' . $var['co'] . '/tip:ok');
                else
                    redirect('cms/posts/grupoNovo/tip:erroGravacao/co:' . $var['co']);
            } else
            {
                if ($ret)
                {
                    redirect('cms/posts/grupoEdita/id:' . $id . '/co:' . $var['co'] . '/tip:ok');
                }
                else
                {
                    redirect('cms/posts/grupoEdita/id:' . $id . '/co:' . $var['co'] . '/tip:erro');
                }
            }
        }
    }
    ...
    function mensagemForm($_var = '')
    {
        ...
        // -
        // -- processa informações -- //
        $user = $this->posts_model->comentario_dados($var['id']);
        // -
        // -- chama as views complementares -- //
        $dados['row'] = $user;
        // echo '<pre>';
    ...
    function mensagemEnvia()
    {
        ...
        else
        {
            ...
            if ($ret)
            {
                redirect('cms/posts/mensagemForm/tip:envioOk/id:' . $var['id']);
            }
            else
            {
                redirect('cms/posts/mensagemForm/tip:erro/id:' . $var['id']);
            }
        }
        // -
        // -- chama as views complementares -- //
    ...
    function tags($_var = '')
    {
        ...
        $modulo = $this->modulo; // infos do módulo
        $modulo['ordenavel'] = 1;
        $conteudo = $this->posts_model->lista_tags($this->var); // dados deste conteúdo reprocessados
...
//        mybug($conteudo);
        ...
        $dados['linkEditar'] = 'cms/posts/tagEdita/co:' . $this->var['co'];
...
//        $this->corpo = $this->layout_cms->barra_navegacao($this->var, $conteudo['ttl_rows'], $navegaOff);
    ...
    function tagSalva()
    {
        ...
        {
            $ret = $this->posts_model->tag_salva($var);
            if ($id == '')
            {
                if ($ret)
                    redirect('cms/posts/tags/co:' . $var['co'] . '/tip:ok');
                else
                    redirect('cms/posts/tagNovo/tip:erroGravacao/co:' . $var['co']);
            } else
            {
                if ($ret)
                {
                    redirect('cms/posts/tagEdita/id:' . $id . '/co:' . $var['co'] . '/tip:ok');
                }
                else
                {
                    redirect('cms/posts/tagEdita/id:' . $id . '/co:' . $var['co'] . '/tip:erro');
                }
            }
        }
    }
