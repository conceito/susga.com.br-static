$(document).ready(function(){
	...
	$('.sim-item').click(function(){
		...
		$.ajax({
			type: "POST",
			url: V['site_url']+"cms/cmsutils/apagaItem/"+id+"/cms_inscritos",
			//data: "id_uf=" + id_uf,
			...
			{						
				if (message == 1){	
					...
					alerta('Houve um erro ao apagar! inscritos.js 67', 'vermelho');
				}
	...
	$('.ativo-item', '#tbl_tab').live('click', function(){
		...
		$.ajax({
			type: "POST",
			url: V['site_url']+"cms/cmsutils/alteraStatus/"+id+"/cms_inscritos/1",
			//data: "id_uf=" + id_uf,
			dataType: ($.browser.msie) ? "text" : "html",			
			beforeSend: function() {
				// msg de carregando				
			...
			{						
				if (message == 1){	
					...
					alerta('Houve um erro de conexão: inscritos.js 98', 'vermelho');
				}				
			}
		});
	...
	$('.inativo-item', '#tbl_tab').live('click', function(){
		...
		$.ajax({
			type: "POST",
			url: V['site_url']+"cms/cmsutils/alteraStatus/"+id+"/cms_inscritos/0",
			//data: "id_uf=" + id_uf,
			dataType: ($.browser.msie) ? "text" : "html",			
			beforeSend: function() {
				// msg de carregando				
	...
	$('.editando-item', '#tbl_tab').live('click', function(){
		...
		$.ajax({
			type: "POST",
			url: V['site_url']+"cms/cmsutils/alteraStatus/"+id+"/cms_inscritos/2",
			//data: "id_uf=" + id_uf,
			dataType: ($.browser.msie) ? "text" : "html",			
			beforeSend: function() {
				// msg de carregando				
	...
	$('textarea[name^=comment-]').blur(function(){
		...
		// via AJAX salva submissão
			$.ajax({
				...
				data: "id="+id+"&comment="+texto+"&op=inscrito",
				dataType: ($.browser.msie) ? "text" : "html",
				...
				{						
					if (message == 1){							
						...
						alerta('Houve um erro de conexão: inscritos.js 196', 'vermelho');
					}	
				}
			});
